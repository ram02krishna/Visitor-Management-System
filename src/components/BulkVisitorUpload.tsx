...
Jane Smith,jane@example.com,+1987654321,Tech Corp,Interview,2024-03-16`
  ...
  const onSubmit = async (formData: BulkUploadFormData) => {
    ...
    try {
      ...
      toast.success(`${visitors.length} visitors uploaded successfully! They will receive QR codes once approved.`)
    } catch (error: unknown) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Bulk Visitor Upload</h1>
          </div>
          <p className="text-sm text-gray-600 dark:text-slate-400 ml-14">
            Upload multiple visitors at once using a CSV file. All visitors will be pending approval.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 shadow-xl rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-800">
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 sm:p-8 space-y-6">
            <div>
              ...
              />
              {errors.approverEmail && <p className="mt-1 text-sm text-red-600">{errors.approverEmail.message}</p>}
            </div>

            <div>
              <label htmlFor="file" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                CSV File *
              </label>
              <input
                type="file"
        ...
        <div className="mt-6 p-6 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 rounded-xl border border-orange-200 dark:border-orange-800">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg p-4 font-mono text-xs border border-orange-200 dark:border-orange-700">
            <div className="grid grid-cols-2 gap-y-2">
              <div>
                <span className="text-orange-600 dark:text-orange-400 font-semibold">name</span> (required)
              </div>
              <div>
                <span className="text-orange-600 dark:text-orange-400 font-semibold">email</span> (required)
              </div>
              <div>
                <span className="text-gray-600 dark:text-slate-400 font-semibold">phone</span> (optional)
              </div>
              <div>
                <span className="text-gray-600 dark:text-slate-400 font-semibold">company</span> (optional)
              </div>
              <div>
                <span className="text-gray-600 dark:text-slate-400 font-semibold">purpose</span> (optional)
              </div>
              <div>
                <span className="text-gray-600 dark:text-slate-400 font-semibold">visit_date</span> (optional, format:
                YYYY-MM-DD)
              </div>
            </div>
          </div>
          <p className="mt-3 text-xs text-gray-600 dark:text-slate-400">
            All uploaded visitors will be in pending status and require approval before receiving QR codes.
          </p>
        </div>
      </div>
    </div>
