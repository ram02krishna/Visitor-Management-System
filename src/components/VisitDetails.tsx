...
export function VisitDetails({ visit, onClose, onUpdate }: VisitDetailsProps) {
  ...
  const toggleBlacklist = async () => {
    ...
    try {
      ...
      toast.success(
        newStatus ? "Visitor successfully Blacklisted" : "Visitor removed from Blacklist"
      );
    } catch (err: unknown) {
      ...
      <div className="relative bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_40px_-12px_rgba(0,0,0,0.2)] dark:shadow-[0_8px_40px_-12px_rgba(0,0,0,0.5)] w-full max-w-sm overflow-hidden animate-scaleIn border border-white dark:border-slate-700 flex flex-col ring-1 ring-black/5 dark:ring-white/10">
        ...
        <div className="px-6 py-5 border-b border-gray-200/50 dark:border-slate-700/50 flex items-center justify-between bg-gradient-to-b from-gray-50/50 to-transparent dark:from-slate-800/50 dark:to-transparent">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-2xl flex items-center justify-center text-white font-bold shadow-lg bg-gradient-to-br ${isBlacklisted ? "from-rose-500 to-red-600 shadow-red-500/30" : "from-blue-500 to-indigo-600 shadow-blue-500/30"}`}
            >
              {isBlacklisted ? (
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[80vh] scrollbar-hide">
          ...
          <div className="flex items-center gap-4 bg-white/50 dark:bg-slate-800/30 p-3 rounded-[2rem] border border-gray-200/50 dark:border-slate-700/50 shadow-sm backdrop-blur-md">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2 sm:col-span-1 p-4 rounded-[1.25rem] bg-gradient-to-br from-gray-50 to-white dark:from-slate-800/80 dark:to-slate-900/80 border border-gray-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow">
              <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1">
            ...
            <div className="col-span-2 sm:col-span-1 p-4 rounded-[1.25rem] bg-gradient-to-br from-gray-50 to-white dark:from-slate-800/80 dark:to-slate-900/80 border border-gray-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-sm font-bold text-gray-800 dark:text-slate-200 uppercase">
            ...
            {visit.vehicle_number && (
              <div className="col-span-2 p-4 rounded-[1.25rem] bg-gradient-to-br from-gray-50 to-white dark:from-slate-800/80 dark:to-slate-900/80 border border-gray-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {visit.vehicle_type && (
                    <span className="inline-flex items-center px-2 py-0.5 rounded-lg bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 text-[10px] font-black uppercase tracking-widest border border-orange-100 dark:border-orange-800/40">
                      {visit.vehicle_type}
                    </span>
                  )}
                </div>
          ...
          {visit.status === "pending" && canApprove && (
            <div className="grid grid-cols-2 gap-2 pt-2">
              <button
                ...
                disabled={loading}
                className="flex items-center justify-center gap-2 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black uppercase tracking-widest text-[9px] transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
              >
                Approve
              </button>
              <button
                ...
                disabled={loading}
                className="flex items-center justify-center gap-2 py-3 border border-gray-200 dark:border-slate-700 text-gray-900 dark:text-white rounded-2xl font-black uppercase tracking-widest text-[9px] transition-all hover:bg-gray-50 active:scale-95 disabled:opacity-50"
              >
                Deny
              </button>
            </div>
          ...
          {visit.status === "checked-in" && isGuardOrAdmin && (
            <button
              ...
              disabled={loading}
              className="w-full py-3.5 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-indigo-700 hover:scale-[1.02] active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20"
            >
          ...
          {isGuardOrAdmin && (
            <button
              ...
              disabled={loading}
              className={`w-full py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                isBlacklisted
        ...
        {visitor?.id_proof_url && (
          <div className="p-4 bg-gray-50/50 dark:bg-slate-800/30 border-t border-gray-100 dark:border-slate-800">
            <a
              ...
              target="_blank"
              className="w-full py-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 rounded-2xl font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-2 hover:bg-gray-50 transition-all shadow-sm"
            >
