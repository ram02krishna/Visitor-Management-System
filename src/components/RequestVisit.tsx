...
export function RequestVisit() {
  ...
  const onSubmit = async (formData: RequestVisitFormData) => {
    ...
    try {
      let photoUrl = null;
      if (formData.photo?.[0])
        photoUrl = await uploadFile(formData.photo[0], "identification-images");

      let idProofUrl = null;
      if (formData.idProof?.[0])
        idProofUrl = await uploadFile(formData.idProof[0], "visitor-documents");
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 pb-12 animate-fadeIn relative">
      ...
      <div className="absolute top-6 left-6 z-50">
        <BackButton
          to="/"
          className="flex items-center gap-3 max-sm:bg-white/80 max-sm:dark:bg-slate-900/80 max-sm:p-1 max-sm:rounded-full max-sm:backdrop-blur-md"
        />
      </div>
      ...
      <div className="max-w-4xl mx-auto mt-12 sm:mt-8">
        ...
        <div className="bg-white dark:bg-slate-900 shadow-xl rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-800">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-4">
                    <label className="block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase">
                      Verification Photos
                    </label>
                        ...
                        <label className="group relative flex flex-col items-center justify-center h-32 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700 hover:border-blue-500 dark:hover:border-blue-400 transition-all cursor-pointer overflow-hidden bg-gray-50/30 dark:bg-slate-800/20">
                          {previewPhoto ? (
                            ...
                            <div className="text-center">
                              ...
                              <span className="mt-2 block text-[10px] font-bold text-gray-400 uppercase">
                                Live Photo
                              </span>
                            </div>
                          )}
                      ...
                      <div className="flex-1">
                        <label className="group relative flex flex-col items-center justify-center h-32 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-400 transition-all cursor-pointer overflow-hidden bg-gray-50/30 dark:bg-slate-800/20">
                          ...
                          <input
                            ...
                            className="sr-only"
                            {...register("idProof")}
                            onChange={(e) => handleFilePreview(e, setPreviewId)}
                          />
                        </label>
                      </div>
                    </div>
            ...
            <div className="mt-6 pt-5 border-t border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row gap-3">
              ...
              >
                {loading ? (
                  ...
                  <>
                    Submit Application & Generate Pass <QrCode size={18} />
                  </>
                )}
              </button>
              <button
          ...
          {success && qrCode && (
            <div
              id="entry-pass-section"
              className="px-4 sm:px-8 pb-8 animate-scaleIn"
            >
                  ...
                  <div className="bg-gradient-to-br from-white to-gray-50 dark:from-slate-900 dark:to-slate-800/80 rounded-[2rem] shadow-[0_24px_60px_-12px_rgba(0,0,0,0.15)] dark:shadow-[0_24px_60px_-12px_rgba(0,0,0,0.6)] overflow-hidden border border-gray-200/80 dark:border-slate-700/60 ring-1 ring-black/[0.03] dark:ring-white/[0.05]">
                    ...
                    <div className="p-6 sm:p-8 flex flex-col items-center">
                      ...
                      <div className="w-full flex items-start justify-between mb-5 sm:mb-7">
                        ...
                        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] sm:text-[10px] font-black tracking-widest uppercase shadow-sm bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800/50">
                      ...
                      <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-5 shadow-md border border-gray-100 dark:border-slate-700 mb-5 sm:mb-7 ring-1 ring-black/[0.04]">
                        <img src={qrCode} alt="Entry Pass QR" className="w-44 h-44 sm:w-52 sm:h-52" />
                      </div>
                      ...
                      <div className="w-full bg-gray-50/80 dark:bg-slate-800/50 rounded-2xl p-4 sm:p-5 mb-5 sm:mb-7 border border-gray-100 dark:border-slate-700/50 space-y-3">
                        <div>
                          ...
                          <p className="text-sm sm:text-base font-black text-gray-900 dark:text-white uppercase truncate">
                            {watch("name")}
                          </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="text-xs font-bold text-gray-700 dark:text-slate-300">
                              {watch("visitDate")
                                ? new Date(watch("visitDate")).toLocaleDateString("en-IN", {
                                    day: "numeric",
                                    month: "short",
                                    year: "numeric",
                                  })
                      ...
                      <div className="w-full grid grid-cols-2 gap-3">
                        <a
                          ...
                          className="flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-slate-800 to-slate-900 dark:from-white dark:to-gray-100 text-white dark:text-slate-900 rounded-xl font-black uppercase tracking-widest text-[10px] sm:text-[11px] hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-slate-900/10"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-2 py-3 border border-gray-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 rounded-xl font-black uppercase tracking-widest text-[10px] sm:text-[11px] hover:bg-gray-50 dark:hover:bg-slate-900 transition-all"
                        >
